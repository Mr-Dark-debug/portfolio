...
import { getAnalyticsOverviewCounts, getAnalyticsSnapshot } from "@/lib/studio/analytics";
...
export default async function StudioDashboardPage() {
  const [articlesResult, analytics, activity, overviewCounts] = await Promise.allSettled([getManagedArticles(), getAnalyticsSnapshot({ days: 30 }), studioActivity(), getAnalyticsOverviewCounts()]);
  ...
  const counts = overviewCounts.status === "fulfilled" ? overviewCounts.value : {};
  const topArticle = snapshot?.topPages.find((row) => typeof row.requestPath === "string" && /^\/[a-z]{2}\/blog\/posts\//.test(row.requestPath));
  const topProject = snapshot?.topPages.find((row) => typeof row.requestPath === "string" && /^\/[a-z]{2}\/projects\//.test(row.requestPath));
  return <>
    ...
    <div className="studio-stat-grid" style={{ marginTop: articlesResult.status === "rejected" ? "1rem" : undefined }}>
      <div className="studio-stat"><span className="studio-stat-label">Views / today</span><strong className="studio-stat-value">{counts.today?.pageviews?.toLocaleString() ?? "—"}</strong><span className="studio-stat-note">Europe/Berlin date</span></div>
      <div className="studio-stat"><span className="studio-stat-label">Views / 7 days</span><strong className="studio-stat-value">{counts.week?.pageviews?.toLocaleString() ?? "—"}</strong><span className="studio-stat-note">Vercel aggregate</span></div>
      ...
      <div className="studio-stat"><span className="studio-stat-label">Most viewed note</span><strong className="studio-stat-value" style={{ fontSize: "1rem" }}>{typeof topArticle?.requestPath === "string" ? <Link href={topArticle.requestPath}>{articles.find((article) => topArticle.requestPath === `/en/blog/posts/${article.slug}`)?.frontmatter.title || topArticle.requestPath}</Link> : "—"}</strong><span className="studio-stat-note">30-day route ranking</span></div>
      <div className="studio-stat"><span className="studio-stat-label">Most viewed project</span><strong className="studio-stat-value" style={{ fontSize: "1rem" }}>{typeof topProject?.requestPath === "string" ? <Link href={topProject.requestPath}>{topProject.requestPath.split("/").pop()}</Link> : "—"}</strong><span className="studio-stat-note">30-day route ranking</span></div>
    </div>
    <div className="studio-dashboard-grid">
      <div className="studio-dashboard-stack">
        ...
        {snapshot?.topPages.length ? <section className="studio-card"><div className="studio-card-header"><div><h2>Top content</h2><p>Most viewed routes in the last 30 days.</p></div></div><div className="studio-list">{snapshot.topPages.slice(0, 5).map((row, index) => typeof row.requestPath === "string" ? <Link className="studio-list-row" href={row.requestPath} key={`${row.requestPath}-${index}`}><span><strong>{row.requestPath}</strong><small>{typeof row.pageviews === "number" ? `${row.pageviews.toLocaleString()} views` : "Views unavailable"}</small></span><ArrowUpRight aria-hidden="true" /></Link> : null)}</div></section> : null}
