...
import { Archive, BarChart3, CalendarClock, Copy, Edit3, Eye, FilePlus2, Filter, MoreHorizontal, Search, Send, Trash2, UploadCloud } from "lucide-react";
...
export default function PostsTable({ initialArticles, viewsBySlug }: { initialArticles: StudioArticleRow[]; viewsBySlug: Record<string, number> }) {
  ...
  }), [articles, query, status, tag, featuredOnly]);
  ...
  const update = async (slug: string, action: "save" | "publish" | "schedule" | "unpublish" | "archive", scheduledAt?: string) => {
    ...
    try {
      ...
      await refresh();
      setMessage(action === "publish" ? "Post published." : action === "schedule" ? "Post scheduled." : action === "archive" ? "Post archived." : "Post saved.");
